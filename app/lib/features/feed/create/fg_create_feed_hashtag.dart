...
class _CreateFeedHashtag extends StatelessWidget {
  const _CreateFeedHashtag();
        ...
        return FeedEditorHashtag(
          ...
          onOpenHashtagEditor: () async {
            ...
            if (filtered.length != selectedHashtags.length) {
              context.read<CreateFeedCubit>().setHashtags(filtered);
            }
          },
          onChanged: (next) {
            context.read<CreateFeedCubit>().setHashtags(next);
          },
